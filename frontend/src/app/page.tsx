...
// --- Icons, Interfaces, Gallery Data (as before) ---
...
export default function HomePage() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    // --- UPDATED & MORE DETAILED FAKE PROGRESS SIMULATION ---
    const steps = [
      { percent: 10, message: "Step 1/5: Initializing...", delay: 2000 }, // 2 seconds
      { percent: 30, message: "Step 2/5: Scraping reference site for style...", delay: 20000 }, // 20 seconds
      { percent: 50, message: "Step 3/5: Parsing your resume with AI...", delay: 20000 }, // 20 seconds
      { percent: 90, message: "Step 4/5: Generating new portfolio with AI...", delay: 120000 }, // 90 seconds (1.5 mins)
      { percent: 95, message: "Step 5/5: Finalizing and deploying your site...", delay: 20000 }, // 20 seconds
    ];
    let currentStep = 0;
    ...
    const runStep = () => {
        ...
        if (currentStep < steps.length) {
            intervalRef.current = setTimeout(runStep, step.delay);
        }
    };

    runStep(); // Start the first step of the simulation
  ...
  return (
    <div className={styles.mainContainer}>
      ...
      {/* --- UPDATED Status, Error, and Result Area --- */}
      {(isLoading || statusMessage || error || finalLink) && (
        <div className={styles.statusMessageContainer}>
            ...
            <>
              ...
              {showUnoLink && (
                <p className={styles.unoLinkMessage}>
                  While you wait (around 2-3 mins), feel free to play a game of <a href="https://play-uno-415351036434.us-east1.run.app/" target="_blank" rel="noopener noreferrer" className={styles.unoLink}>UNO that I built!</a>
                </p>
              )}
            </>
          )}
          ...
          {!isLoading && statusMessage && !error && (
            <>
              ...
              {finalLink && (
                <a href={finalLink} target="_blank" rel="noopener noreferrer" className={styles.resultLinkButton}>
                  View Your Portfolio ✨
                </a>
              )}
            </>
